...
const Annotator: React.FC<{ bookKey: string; contentInsets: Insets }> = ({
  ...
  // Latches the instant quick action so a long-press's multiple selectionchange
  // events (iOS re-confirms the native selection after our deselect()) run it
  ...
  const onLoad = (event: Event) => {
    ...
    // Re-arm the instant quick action at the start of each gesture: a single
    // long-press emits multiple selectionchange events for the same word —
    // without re-arming, the system-dictionary sheet stacked twice (the action
    // fired once per event instead of once per gesture).
  ...
  // viewport; resolve the visible section's doc/index, then select the word
  // under the point.
  useEffect(() => {
  ...
  const handleQuickAction = () => {
    // Only fire from a long-press hold. Without this a tap-to-deselect after
    // dismissing the system dictionary occasionally re-opened it off a racy
    // lingering selectionchange.
    ...
    runOnce(deferredQuickActionRef.current, runAction);
  };
  ...
  const handleShare = () => {
    ...
    const position = trianglePosition
      ...
      : undefined;
    void shareSelectedText(selection.text, position);
    handleDismissPopupAndSelection();
  };
