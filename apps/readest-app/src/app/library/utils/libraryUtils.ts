 ...
 * (the library's sort-by-updated key) and `readingStatusUpdatedAt`. Use this
 * for every deliberate status edit so the timestamp is never forgotten.
 */
...
export const getBookContextMenuItemIds = (book: Book): BookContextMenuItemId[] => {
  ...
  ids.push('showDetails', 'showInFinder', 'searchGoodreads', 'delete');
  return ids;
};
