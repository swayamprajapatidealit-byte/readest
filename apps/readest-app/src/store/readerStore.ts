...
interface ViewState {
  ...
  /* True while the reader is showing a position requested by an external
     ...
     The progress auto-save skips while this is true so the user's actual
     last-read position isn't overwritten by a preview. Cleared on the first
     user-initiated relocate (page turn / scroll). */
  previewMode: boolean;
