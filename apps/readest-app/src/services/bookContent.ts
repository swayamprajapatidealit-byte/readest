...
// Legacy OPDS partial-download stream marker (`pse://...`) and RSS feed-book
// marker (`feed://...`). Kept as pure prefix checks so old library rows with
// these `book.url` schemes still resolve to a sane source kind instead of
// `missing` — OPDS/RSS import itself has been removed.
const PSE_SCHEME = 'pse://';
const isPseStreamFileName = (name: string): boolean => name.startsWith(PSE_SCHEME);
const FEED_SCHEME = 'feed://';
const isFeedBookUrl = (url: string): boolean => url.startsWith(FEED_SCHEME);
