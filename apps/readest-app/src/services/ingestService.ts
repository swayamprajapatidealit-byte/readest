...
export async function ingestFile(
  ...
  //
  // Reject URLs and content URIs defensively. The byFilePath index only
  // carries real on-disk paths, but `inPlace` could in principle be set on a
  // non-path source by a buggy caller.
  if (
    inPlace &&
    !opts.transient &&
    opts.lookupIndex &&
