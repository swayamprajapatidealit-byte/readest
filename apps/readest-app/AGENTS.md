...
Readest is a **Next.js 16** web ebook reader, deployed to Cloudflare Workers. It's part of a pnpm monorepo at `/apps/readest-app/`. This fork is web-only — no Android, iOS, macOS, Windows, or Linux desktop app support.
...
| `src/components/` | React components (reader, settings, library, dictionary, etc.) |
| `src/services/`   | Business logic: TTS, dictionaries, WordLens, RSVP, metadata lookup, local DB |
| `src/store/`      | Zustand state stores                                          |
| `src/hooks/`      | Custom React hooks                                            |
| `src/libs/`       | Document loaders, storage, app-lock crypto                    |
| `src/utils/`      | Pure utility functions                                        |
| `src/types/`      | TypeScript type definitions                                   |
| `src/context/`    | React Context providers (Env, Dropdown, PostHog telemetry)    |
...
Always use `pnpm worktree:new <branch-name|pr-number>` to create worktrees. Never use `git worktree add` directly — the script handles submodule initialization (simplecc WASM, foliate-js), dependency installation, `.env` copying, and vendor assets required for lint to pass.
...
- `.agents/rules/` — project rules for TypeScript, verification, and related workflows

## Project Rules

Rules are in `.agents/rules/`: typescript, verification.
